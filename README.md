...
관리 웹은 VPN 서버 상태 확인, 클라이언트 인증서 발급·등록·삭제, 접속 상태 및 최근 이벤트 확인에 사용합니다.

### 서버 상태

첫 화면에서 웹 콘솔과 VPN 서버 제어 소켓 연결 상태, VPN 서버 주소, DTLS 전송 방식과 TUN MTU를 확인합니다. `Live`와 `정상 운영 중`이 표시되고 서버 주소·MTU가 정상적으로 보이면 관리 웹이 VPN 서버의 제어 채널에 연결된 상태입니다.

![Autobricks VPN 관리 웹 서버 개요](images/autobricks-vpn-web-001.png)

### 클라이언트 관리

클라이언트 표에는 등록된 VPN 주소, 인증서 SHA-256 지문, 현재 접속한 외부 IP와 연결 상태가 표시됩니다.

![Autobricks VPN 클라이언트 관리](images/autobricks-vpn-web-002.png)

- **추가**: 클라이언트 이름, VPN 주소와 인증서 유효 기간을 입력합니다. 선택적으로 허용 접속 IP 대역을 CIDR로 입력하면 클라이언트 인증서의 `urn:autobricks:allowed-source-cidr:` URI SAN에 기록합니다. 로그인 ID와 암호는 둘 다 입력하거나 둘 다 비워야 합니다.
- **발급 및 다운로드**: 서버의 Intermediate CA로 인증서를 발급하고 `[client]` 설정과 인증서·개인키·신뢰 체인을 포함한 `<클라이언트 이름>-client.ini` 하나를 내려받습니다. 선택한 로그인 ID와 암호는 서버 등록 정보에만 저장되고 다운로드한 INI에는 포함되지 않습니다.
- **삭제**: 클라이언트 등록을 제거하고 VPN 서버 설정을 즉시 다시 읽습니다. 화면의 확인 창에서 승인해야 실행됩니다.

다운로드한 INI에는 클라이언트 개인키가 있으므로 소유자만 읽을 수 있게 보관하고 공개 저장소나 메신저에 올리지 마세요. 같은 VPN 주소를 중복 등록하거나 다른 주소에 같은 인증서 지문을 배정할 수 없습니다.

#### 클라이언트 접속 IP 제한

**허용 접속 IP 대역**은 클라이언트 인증서에 접속 출발지 정책을 포함하는 선택 항목입니다. 값은 IPv4 CIDR로 입력하며, 단일 공개 IP만 허용하려면 `/32`, 공개 IP 대역을 허용하려면 해당 네트워크의 접두사 길이를 사용합니다. 예를 들어 문서용 대역 `192.0.2.0/24`를 입력하면 인증서에 다음 URI SAN이 발급됩니다.

```text
urn:autobricks:allowed-source-cidr:192.0.2.0/24
```

VPN 서버는 DTLS 인증이 완료될 때 UDP 소켓에서 관측한 클라이언트 IPv4 주소를 이 CIDR과 비교합니다. 클라이언트가 NAT 뒤에 있으면 장치의 사설 IP가 아니라 **NAT 이후 서버에 보이는 공개 IP**가 검증 대상입니다.

- URI SAN이 없으면 출발지 IP에 대한 추가 제한을 적용하지 않습니다.
- URI SAN이 하나 있으면 관측한 출발지 IP가 CIDR에 포함될 때만 연결합니다.
- CIDR 밖에서 접속하거나 CIDR 형식이 잘못된 경우 연결을 거부합니다.
- host bit가 설정된 네트워크 주소, 중복된 CIDR 정책 또는 알 수 없는 `urn:autobricks:` 정책도 거부합니다.
- 이 정책은 인증서에 저장되며 다운로드한 `client.ini`의 `[client]` 설정 항목을 추가하거나 변경하지 않습니다.

### 최근 이벤트

최근 이벤트는 VPN 서버 시작·중지와 클라이언트 접속·해지를 최신순으로 보여줍니다. 접속 상태는 Server-Sent Events로 즉시 갱신되며, **새로고침** 버튼으로 현재 기록을 다시 불러올 수 있습니다. 공개 문서용 화면에서는 외부 IP를 마스킹했습니다.

![Autobricks VPN 최근 이벤트](images/autobricks-vpn-web-003.png)
